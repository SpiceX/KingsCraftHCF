...
class CustomEnchant extends Enchantment
{
    ...
    public function __construct(HCF $plugin, int $id)
    {
        ...
        $this->extraData = $plugin->getEnchantmentManager()->getEnchantmentData($this->name, "extra_data", $this->getDefaultExtraData());
        $this->cooldownDuration = (int)$plugin->getEnchantmentManager()->getEnchantmentData($this->name, "cooldowns", $this->cooldownDuration ?? 0);
        $this->chance = (int)$plugin->getEnchantmentManager()->getEnchantmentData($this->name, "chances", $this->chance ?? 100);
        foreach ($this->getDefaultExtraData() as $key => $value) {
            if (!isset($this->extraData[$key])) {
                $this->extraData[$key] = $value;
                $plugin->getEnchantmentManager()->setEnchantmentData($this->name, "extra_data", $this->extraData);
            }
        }
