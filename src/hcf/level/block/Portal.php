...
class Portal extends Transparent {
    ...
    public function onEntityCollide(Entity $entity): void {
        ...
        if($entity->getLevel()->getName() === $server->getDefaultLevel()->getName()) {
            $spawn = $server->getLevelByName("nether")->getSpawnLocation();
        }
        else {
        ...
        HCF::getInstance()->getScheduler()->scheduleDelayedTask(new class($spawn, $entity) extends Task {
            ...
            public function onRun(int $currentTick) {
                ...
                if($this->player->getLevel()->getBlock($this->player)->getId() !== Block::PORTAL) {
                    $this->player->setChangingDimensions(false);
                    return;
                }
                $dimension = DimensionIds::NETHER;
                if($this->player->getLevel()->getFolderName() === "nether") {
