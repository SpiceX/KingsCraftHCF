...
class PardonCommand extends Command {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void {
        if(($sender instanceof HCFPlayer and $sender->hasPermission("permission.admin"))
            ...
            $stmt = $this->getCore()->getMySQLProvider()->getDatabase()->prepare("SELECT effector, reason, expiration FROM bans WHERE username = ?;");
            ...
            $stmt = $this->getCore()->getMySQLProvider()->getDatabase()->prepare("DELETE FROM bans WHERE username = ?;");
            $stmt->bind_param("s", $args[0]);
            $stmt->execute();
            $stmt->close();
