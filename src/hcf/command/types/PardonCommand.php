...
class PardonCommand extends Command
{
    ...
    {
        parent::__construct("pardon", "Pardon a player.", "/pardon <player>", ["unban"]);
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): void
    {
        if ($sender instanceof ConsoleCommandSender
            ...
            $stmt = $this->getCore()->getMySQLProvider()->getDatabase()->prepare("SELECT effector, reason, expiration FROM bans WHERE username = :username;");
            ...
            $stmt->closeCursor();
            $stmt = $this->getCore()->getMySQLProvider()->getDatabase()->prepare("DELETE FROM bans WHERE username = :username;");
            $stmt->bindParam(":username", $args[0]);
            $stmt->execute();
            $stmt->closeCursor();
