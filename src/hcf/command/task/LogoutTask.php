...
class LogoutTask extends Task {
    ...
    public function onRun(int $currentTick) {
        ...
        if($this->player->getFloorX() !== $this->position->getFloorX() or $this->player->getFloorY() !== $this->position->getFloorY() or $this->player->getFloorZ() !== $this->position->getFloorZ()) {
            ...
            $this->player->sendTitle(TextFormat::DARK_RED . "Failed to log out", TextFormat::GRAY . "You must stand still!");
            HCF::getInstance()->getScheduler()->cancelTask($this->getTaskId());
            return;
        }
        if($this->time >= 0) {
            $this->player->sendTitle(TextFormat::DARK_GREEN . "Logging out in", TextFormat::GRAY . "$this->time seconds" . str_repeat(".", ($this->maxTime - $this->time) % 4));
            $this->time--;
            return;
        }
