...
class WatchdogListener implements Listener {
    ...
    public function onPlayerMove(PlayerMoveEvent $event): void {
        ...
        if((!isset($this->phasingMessages[$player->getRawUniqueId()])) or $this->phasingMessages[$player->getRawUniqueId()] !== time()) {
            /** @var Block[] $blocks */
            $blocks = [];
            ...
            foreach($blocks as $block) {
                ...
                if($block instanceof Fallable or $block instanceof Ladder or $block->canPassThrough() === true or ($block instanceof FenceGate and ($block->getDamage() & 0x04) > 0) or ($block instanceof Trapdoor and $block->getDamage()> 8)) {
                    continue;
                }
                ...
                foreach($this->core->getServer()->getOnlinePlayers() as $onlinePlayer) {
                    if($onlinePlayer->getGroup()->getIdentifier() >= GroupManager::TRAINEE and $onlinePlayer->getGroup()->getIdentifier() <= GroupManager::OWNER) {
    ...
    public function onPlayerQuit(PlayerQuitEvent $event): void {
        ...
        if($player->isFrozen()) {
            ...
            $stmt = $this->core->getMySQLProvider()->getDatabase()->prepare("INSERT INTO bans(uuid, username, effector, reason, expiration) VALUES(?, ?, ?, ?, ?);");
            $stmt->bind_param("ssssi", $uuid, $name, $effector, $reason, $expiration);
            $stmt->execute();
            $stmt->close();
            $time = 604800;
